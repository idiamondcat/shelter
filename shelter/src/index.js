import pets from 'pets-list.json';
...
const activeLink = document.getElementsByClassName('nav__item nav__item--active')[0];
...
linksContainer.addEventListener('click', function addActiveElem(event) {
  ...
  //   const activeElem = this.querySelector('.nav__item--active');
  //   if (activeElem) {
  ...
  //   linksContainer.children[i].classList.remove('nav__item--active');
  // }
  // evt.target.parentNode.classList.add('nav__item--active');
});
...
burgerBtn.addEventListener('click', (evt) => {
  ...
  //   activeLink.addEventListener('click', openMenu());
  // } else {
  //   // overlay.removeEventListener('click', openMenu());
  //   activeLink.removeEventListener('click', openMenu());
  // }
});
...
if(activeLink) {
  ...
  activeLink.addEventListener('click', function linkEvt(event) {
    ...
    if (burgerBtn.classList.contains('nav__btn--active')) {
      ...
      activeLink.removeEventListener('click', linkEvt(event));
    }
  });
}

// links.forEach(elem => {
  // activeLink.addEventListener('click', function linkOnClick(event) {
  //     event.stopPropagation();
  //       if (burgerBtn.classList.contains('nav__btn--active')) {
  //         event.stopPropagation();
  //         openMenu();
...
const toLeft = () => {
  // let newTemp = currArr;
  // currArr = prevArr, prevArr = newTemp;
  // nextArr = generateAnotherPetsLeft(nextArr, currArr);
  // return [prevArr, currArr, nextArr];
  nextArr = [];
...
function init() {
  ...
  console.log('Все требования выполнены. Итого 110/110');
}